...
def apply_filters_and_calculate_metrics(noisy_image):
    ...
    bilateral_filter = cv2.bilateralFilter(noisy_image, 9, 75, 75)  # Add bilateral filter
    ...
    mse_bilateral, snr_bilateral, psnr_bilateral = calculate_metrics(image, bilateral_filter)  # Calculate metrics for bilateral filter
...
for i, sigma in enumerate(sigmas, start=1):
    # Add Gaussian noise with the current sigma value
    mean = 0
    var = sigma ** 2
    ...
    (avg, maxf, minf, bilateral) = apply_filters_and_calculate_metrics(noisy_image)  # Include bilateral filter result

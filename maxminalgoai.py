...
def minimax(node, depth, maximizingPlayer):
    if depth == 0 or node.is_terminal():  # Base case: Terminal node
        return node.evaluate() or 0  # If node evaluation is None, return 0

    if maximizingPlayer:
        best_value = float('-inf')
        for child in node.get_children():
            value = minimax(TicTacToe(child), depth - 1, False)
            best_value = max(best_value, value)
        return best_value
    else:
        best_value = float('inf')
        for child in node.get_children():
            value = minimax(TicTacToe(child), depth - 1, True)
...
def find_best_move(board):
    ...
    for child in board.get_children():
        value = minimax(TicTacToe(child), 5, False)  # Depth set to 5 for example
        if value > best_value:
            best_value = value
            best_move = child
    return best_move
